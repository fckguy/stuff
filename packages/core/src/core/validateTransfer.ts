...
export async function validateTransfer(
...
): Promise<DecodedTransferCheckedInstruction> {
  ...
  if (
    !transaction.signatures
      ...
      .find((pubkey) => pubkey.equals(owner.pubkey))
  )
    throw new Error("owner missing signature");
  ...
  if (isTransferCheckedInstruction(instruction)) {
    ...
    (instruction as unknown as DecodedTransferCheckedInstruction).keys.mint = {
      pubkey: token.mint,
      isSigner: false,
      isWritable: false,
    };
    (
      instruction as unknown as DecodedTransferCheckedInstruction
    ).data.decimals = token.decimals;
  }

  return instruction as unknown as DecodedTransferCheckedInstruction;
}
